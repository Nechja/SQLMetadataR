...
```python
from tools.sql_explorer import SQLExplorer

# Initialize explorer with a database path
explorer = SQLExplorer("path/to/database.db")

# Extract metadata
db = explorer.extract_metadata(
    sample_rows=5,          # Number of sample rows to extract per table
    max_column_values=10,   # Maximum number of distinct values to sample per column
    execute_queries=True,   # Whether to execute example queries
    query_result_limit=5    # Maximum number of rows to return in query results
)

# Save metadata to a file
db.save_to_file("database_metadata.json")

# Or run the complete workflow in one step
explorer.run_workflow(output="database_metadata.json")
```

### Command-Line Usage

SQLMetadataR can also be used from the command line:

```bash
python -m tools.cli path/to/database.db --output metadata.json
```

Command-line options:
- `--output`, `-o`: Output JSON file path
- `--sample-rows`, `-r`: Number of sample rows to include per table
- `--max-values`, `-v`: Maximum distinct values to sample per column
- `--no-execute`: Skip query execution
- `--query-results`, `-q`: Number of result rows to include for executed queries